...
export const DEFAULT_AI_ANSWER_TEMPLATE =
  'これはRedmineチケットの本文と変更履歴のテキストです。あなたの回答はそのままRedmineのカスタムフィールドに保存されるため、前置き・挨拶・Markdown装飾（見出しや箇条書き記号など）を使わず、結論から簡潔な地の文で400文字程度に要約してください。誰が何をしたかの主語がわかるようにしてください。わからない時は詳細不明でもよいです。結論としてどうなったか、どういう状態にあるかを優先して記載してください。子チケットがある場合は存在することを簡単に記載してください。また、まず日本語で次に英語で回答してください。日本語回答と英語回答の間は、■■English■■で区切ってください。';

// クローズ扱いのステータスのチケットに対して使う定型文（対応が確定している前提のまとめ方）
export const DEFAULT_AI_ANSWER_CLOSED_TEMPLATE =
  'これは対応完了したRedmineチケットの本文と変更履歴のテキストです。あなたの回答はそのままRedmineのカスタムフィールドに保存されるため、前置き・挨拶・Markdown装飾（見出しや箇条書き記号など）を使わず、以下の項目を使って内容をまとめてください。項目は【】で囲った個所です。まとめと次の項目の間には改行を挟んでください。括弧（）で囲んだ箇所が、あなたにまとめてほしい内容です。チケットはQA/BUG/Change Request/Enhanceと種類が分かれるため、該当する情報がない場合もあります。その場合は情報の無い項目は記載しないでください。子チケットがある場合は存在することを【問い合わせ概要】に記載してください。また、まず日本語で次に英語で回答してください。日本語回答と英語回答の間は、■■English■■で区切ってください。英語での項目は【Inquiry Summary】【Issue】【Cause】【Scope of Impact】【Action Taken】【User Action】【Result】としてください。\n\n【問い合わせ概要】\n（全体を短くまとめる。項目「App ID/PGM ID」に値がある場合はPGM IDとアプリの名前も書く）\n\n【事象】\n（起票の元になった出来事をBUGの場合は項目「Incident Details」と「description」を元に記載する。具体的な事象、再現条件、画面や機能単位の情報をまとめる。CRの場合は変えたい挙動を書く）\n\n【原因】\n（BUGの場合は項目「Cause」から記載する。技術的・業務的な原因や背景。コード上の問題、設定不備、仕様の認識違いなどをまとめる。）\n\n【影響範囲】\n（BUGの場合は項目「Scope of Impact」「Business Impact」から記載する。どの機能・画面・ユーザ・環境に影響したか、影響の有無と範囲をまとめる。）\n\n【対応内容】\n（実施した修正内容、設定変更、リリース作業など、技術的・運用的な対応の詳細をまとめる。）\n\n【ユーザ対応】\n（ユーザー側で行ったアクションをまとめる。）\n\n【結果】\n（QAの場合はユーザー側への回答をまとめる。BUG,CR,ENの場合はプログラム、業務がどのように変わったかまとめる）';
