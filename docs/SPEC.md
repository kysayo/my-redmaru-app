...
Redmineチケットページの「AI回答更新」ボタンをクリックすると、AIチャットへの送信・回答完了待ち・Redmineへの書き戻しまでを自動で行う。人間による送信前後の確認ゲートは設けない。
...
- **将来のPlaywrightバッチとの関係**: このボタンのクリック起点フロー自体がPlaywrightバッチ（別アプリ、本リポジトリ外）からも再利用される想定。拡張機能側にPlaywright専用のコードパスは作らない。バッチ側の完了検知は本機能の `AUTO_ANSWER_STATUS` 通知に依存せず、Redmine REST APIのポーリングで行う設計を前提とする。
