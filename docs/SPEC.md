...
以下の **2ファイル** の `DEFAULT_TEMPLATE` 定数を同じ内容に揃えて変更し、ビルドし直す。

| ファイル | 役割 |
|---------|------|
| `entrypoints/background.ts` | ユーザーが一度も設定を保存していない場合に使われる初期値 |
| `entrypoints/options/App.vue` | 設定ページを開いたとき表示されるデフォルト値 |

> **注意**: 一方だけ変更すると、設定ページの表示と実際の送信内容がずれる場合がある。Vue コンポーネントが background.ts に依存せず独立してストレージを読んでいる構造のため。
