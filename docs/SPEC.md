...
- ボタンクリックで以下の情報を Redmine API（`/issues/{id}.json?include=journals`）で取得する
  ...
  - 子チケット（番号・題名のみ。詳細は「子チケット情報の付加」参照）
  - コメント（journals のうち notes が空でないもの）
- Redmine API の認証キーは `ViewCustomize.context.user.apiKey` から取得する（MAIN World ブリッジ経由）

#### 子チケット情報の付加

表示中のチケットに子チケットがいる場合、「to MaruCha」と「AI回答更新」で送信するチケット情報に子チケットの一覧を含める（「for TR」は移送申請の項目抽出用のため対象外）。子チケットの説明文やカスタムフィールドまで含めると情報量が増えすぎて要約の精度がかえって落ちるため、**番号と題名のみ**に絞る。

- `fetchChildIssues()` が `GET /issues.json?parent_id={id}&status_id=*&limit=100` で子チケット一覧を取得する。`status_id=*` を付けないと `/issues.json` は既定で未クローズのチケットしか返さないため、クローズ済みの子チケットも一覧に含まれるよう明示的に指定している
...
- **自動送信・完了検知・回答抽出**: `aichat.content.ts` が `entrypoints/shared/aichatDom.ts` の関数を使い、テキスト挿入後にメッセージを送信し、回答完了を検知する。タイムアウト時は書き込みを行わない（誤って古い/中途半端な回答を書き込む方が害が大きいため）。
  ...
  - **完了検知**: `waitForAnswerComplete()` は監視開始時点のアンケート個数・回答コンテナ個数を**ベースライン**として記録し、そこから**増加した**ときのみ完了と判定する（存在有無ではなく差分で見る）。また監視開始から最低5秒（`minWaitMs`）は判定を確定させない。
    - 判定の根拠は二段構え。第一に「この回答は役に立ちましたか？」フィードバックアンケートの出現数の増加（devtools実機確認済み。ただし該当spanは `class="hidden ..."` を持ち、**生成完了前からCSSで非表示のままDOMに存在している場合がある**ため、存在有無だけで判定すると送信直後に誤検知する。ベースライン差分方式はこの誤検知対策）。第二にフォールバックとして、回答コンテナ数の増加 + デバウンス（既定1800ms）でDOM変化が止まったことを検知する方式を残す。

        **判定は300ms間隔のポーリングで駆動する（`MutationObserver` のコールバック内では行わない）。** 当初は各判定をObserverのコールバック内で実行していたが、誤検知防止の最小待機（`minWaitMs`、既定5秒）以内に生成が完了してDOM変化が止まると、その後Observerが二度と発火せず、判定が再実行されないままハードタイムアウトする問題があった。画面には回答もアンケートも出ているのにタイムアウト扱いになる。入力・出力がともに短い英訳ジョブ（バッチの`translate-en`）で頻発したため、DOM変化に依存しない駆動に変更した。Observerは「最後にDOMが動いた時刻」の記録（デバウンス判定用）だけに使う。

        タイムアウト上限は設定ページの「AI回答」タブで秒数を変更できる（ストレージキー: `aiAnswerTimeoutSeconds`、デフォルト90秒）。`background.ts` の `handleAutoAnswerRequest` がstorageから読み取り、`AUTO_ANSWER_START` の `timeoutMs`（ミリ秒換算）として `aichat.content.ts` に渡す。
...
- どちらの定型文にも個別の「デフォルトに戻す」ボタンがある（テキストエリアへの復元のみで保存は手動。他タブと同様）。`chrome.storage.sync`は拡張機能の更新では自動更新されないため、コード側のデフォルト文言を変えても一度保存済みの値は残り続ける。このボタンで手動更新できるようにしている
